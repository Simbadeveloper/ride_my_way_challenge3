...
class TestRegistration(Base):
    ...
    def test_user_already_exists(self): 
        ...
        response2 = self.client.post('/api/v2/auth/signup',data=json.dumps(self.user1_data), content_type='application/json')
        result = json.loads(response2.data)
        self.assertEqual(result['message'],"A user with that username exists!")

    def test_user_already_exists_with_an_email(self): 
        """  Method for testing that registration of a user is only once(email).  """
        
        response1 = self.client.post('/api/v2/auth/signup',data=json.dumps(self.user1_data), content_type='application/json')
        self.user1_data={"username":"Nana", "firstname":"Snyder", "lastname":"Mbishai", "email":"mbish@gmail.com", "password":"123456", "confirm_pwd":"123456"}
        response2 = self.client.post('/api/v2/auth/signup',data=json.dumps(self.user1_data), content_type='application/json')
        result = json.loads(response2.data)
        self.assertEqual(result['message'],"A user with that email already exists!")
